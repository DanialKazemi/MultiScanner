...
public class PolygonView extends FrameLayout {
    ...
    private void initPaint() {
        paint = new Paint();
        paint.setColor(ContextCompat.getColor(context,R.color.line_blue));
    ...
    public Map<Integer, PointF> getPoints() {
        ...
        Map<Integer, PointF> pointFMap=new ArrayMap<>();
        pointFMap.put(0,points.get(0));
        pointFMap.put(1,points.get(1));
        pointFMap.put(2,points.get(2));
        pointFMap.put(3,points.get(3));
    ...
    public Map<Integer, PointF> getOrderedPoints(List<PointF> points) {
        ...
        if (points.get(0).y!=0 && points.get(2).y!=0) {
            for (PointF pointF : points) {
                pointsY.add(pointF.y);
            }
            Collections.sort(pointsY);

            PointF y0 = new PointF();
            PointF y1 = new PointF();
            PointF y2 = new PointF();
            PointF y3 = new PointF();

            for (PointF pointF : points) {
                if (pointF.y == pointsY.get(0) && y0.equals(0,0)) y0 = pointF;
                else if (pointF.y == pointsY.get(1) && y1.equals(0,0)) y1 = pointF;
                else if (pointF.y == pointsY.get(2) && y2.equals(0,0)) y2 = pointF;
                else if (pointF.y == pointsY.get(3) && y3.equals(0,0)) y3 = pointF;
            }

            if (y0.x > y1.x) {
                orderedPoints.put(0, y1);
                orderedPoints.put(1, y0);
            } else {
                orderedPoints.put(0, y0);
                orderedPoints.put(1, y1);
            }

            if (y2.x > y3.x) {
                orderedPoints.put(2, y3);
                orderedPoints.put(3, y2);
            } else {
                orderedPoints.put(2, y2);
                orderedPoints.put(3, y3);
            }
        }
    ...
    private class MidPointTouchListenerImpl implements OnTouchListener {
        ...
        private MidPointTouchListenerImpl(ImageView mainPointer1, ImageView mainPointer2) {
            ...
            switch (eid) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (isValidShape(getPoints())) {
                        color =  ContextCompat.getColor(context,R.color.line_blue);
                    } else {
                        color = ContextCompat.getColor(context,R.color.line_orange);
                    }
                    paint.setColor(color);

                    ScanFragment.frameLayout.buildDrawingCache();
                    Bitmap cache = ScanFragment.frameLayout.getDrawingCache();
                    ScanFragment.contourItems.set(ScanFragment.chosenIndex, Bitmap.createBitmap(cache));
                    ScanFragment.adapter.notifyDataSetChanged();
                    ScanFragment.frameLayout.destroyDrawingCache();
            ...
            switch (eid) {
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (((StartPT.x + mv.x + v.getWidth()) < polygonView.getWidth() && (StartPT.y + mv.y + v.getHeight() < polygonView.getHeight())) && ((StartPT.x + mv.x) > 0 && StartPT.y + mv.y > 0)) {
                        ...
                        StartPT = new PointF(v.getX(), v.getY());
                        event.setLocation(v.getX(),v.getY());
